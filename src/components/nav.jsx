...
export default function Botoncito() {
  ...
  return (
    <nav className="flex flex-col bg-gray-800 border border-purple-900/50 shadow-lg rounded-[30px] backdrop-blur-lg h-full p-6 justify-center items-center space-y-18">
      {navItems.map((item, index) => (
        <div key={index} className="relative group">
 <a
      href="#"
      className={`flex items-center justify-center w-20 h-20 rounded-xl p-3 transition-all duration-300 ease-in-out ${
        activeItem === index ? "scale-110 bg-purple-500/10" : "group-hover:scale-125"
      }`}
